...
class CartItems extends HTMLElement {
  ...
  updateProgressBar(cartTotal, itemCount) {
    ...
    const preGoalMessageTemplate =
      progressWrapper.dataset.preGoalMessageTemplate;
    ...
    if (itemCount === 0 || cartTotal === 0) {
      ...
      if (goalMessageElement) {
        ...
        const remainingAmountFormatted = moneyFormat.replace(
          "{{amount_with_comma_separator}}",
          remainingAmount.toFixed(2)
        );
        const preGoalMessage = preGoalMessageTemplate.replace(
          "[remainingForGoalFormatted]",
          remainingAmountFormatted
        );

        goalMessageElement.innerHTML =
          remainingForGoal > 0 ? preGoalMessage : postGoalMessage;
      }
    }
  }
